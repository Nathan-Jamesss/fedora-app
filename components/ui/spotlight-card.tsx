...
interface GlowCardProps {
    children: ReactNode;
    className?: string;
    glowColor?: 'green' | 'emerald' | 'blue' | 'purple';
}

const glowColorMap = {
    green: { base: 120, spread: 160 },
    emerald: { base: 150, spread: 140 },
...
const glowBeforeAfter = `
  ...
  [data-glow]::before{background-image:radial-gradient(calc(var(--spotlight-size)*0.75) calc(var(--spotlight-size)*0.75) at calc(var(--x,0)*1px) calc(var(--y,0)*1px),hsl(var(--hue,120) calc(var(--saturation,100)*1%) calc(var(--lightness,50)*1%)/var(--border-spot-opacity,1)),transparent 100%);filter:brightness(2);}
  [data-glow]::after{background-image:radial-gradient(calc(var(--spotlight-size)*0.5) calc(var(--spotlight-size)*0.5) at calc(var(--x,0)*1px) calc(var(--y,0)*1px),hsl(0 100% 100%/var(--border-light-opacity,0.6)),transparent 100%);}
...
export const GlowCard: React.FC<GlowCardProps> = ({ children, className = '', glowColor = 'green' }) => {
    ...
    const styles = {
        ...
        '--radius': '16',
        '--border': '2',
        '--backdrop': 'rgba(10,22,14,0.7)',
        '--backup-border': 'rgba(34,197,94,0.18)',
        '--size': '220',
        '--outer': '1',
        '--border-size': 'calc(var(--border,2)*1px)',
        '--spotlight-size': 'calc(var(--size,150)*1px)',
        '--hue': `calc(${base} + (var(--xp,0)*${spread}))`,
        backgroundImage: `radial-gradient(var(--spotlight-size) var(--spotlight-size) at calc(var(--x,0)*1px) calc(var(--y,0)*1px),hsl(var(--hue,120) 80% 50%/0.08),transparent)`,
    ...
    return (
        <>
            <style dangerouslySetInnerHTML={{ __html: glowBeforeAfter }} />
            <div ref={cardRef} data-glow style={styles}
                className={`rounded-2xl relative backdrop-blur-md shadow-[0_2rem_3rem_-1rem_rgba(0,0,0,0.5)] ${className}`}>
                <div data-glow />
                {children}
            </div>
        </>
